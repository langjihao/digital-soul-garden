...
│   ├── PhotoTimeline/TravelMap.vue    # /life 时间轴相册 + 中国旅行地图（v9）
...
├── api/life/{album.get,photo/[id].get}.ts  # 相册数据 / 照片代理（磁盘缓存）
...
| `ALBUM_*` | /life 相册与旅行地图的 couple-album 后台服务账号（v9） |
...
11. **中国地图资产是预投影产物**：`app/assets/china-map.json` 由 `scripts/build-china-map.mjs` 生成（墨卡托+抽稀+南海诸岛插框），**南海 feature 的 name 为空，处理时别按 name 过滤**；城市定位需与组件内投影公式一致
12. **dev 调试 /life 需手动 `source /etc/garden.env`**：`restart-dev.sh` 不加载 env 文件

## 7. 当前待办

- [ ] giscus app 安装（github.com/apps/giscus → 选本仓库），装完评论即可收发；env 四件套已配置
- [ ] /life 转正：豆瓣已实配；微信读书/Keep 凭证未配；**每日同步 cron 尚未建立（数据是静态快照）**，需手动建 `/etc/cron.d/garden-life-sync` 定时 POST `/api/life/sync`
- [ ] 相册接真实空间：当前连的是演示 couple space；正规流程=相册端生成邀请码 → 服务账号 join，勿直接操作数据库
- [ ] 站点监控告警（当前无存活监测）
...
| v9 | /life 时间轴相册 + 中国旅行地图（接 couple-album 后台，照片服务端代理） |
